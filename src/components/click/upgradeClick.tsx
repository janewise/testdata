...
const UpgradeClick: FunctionComponent<ButtonProps> = (props: ButtonProps) => {
  ...
  useEffect(() => {
    ...
    const levelCondition = props.level < 20;
    ...
    <div className="upgrade_box">
      <HtmlTooltip placement={isMobile ? 'bottom' : 'left'} title={
          <Typography variant="body2" align="left">
            {props.name} <br/>
            Max: {props.level}/19 <br/>
            <div style={{display: props.level === 0 ? "none": "inline-block"}}>
              <hr />
              {"Each " + props.name + " add "} <b> +1 </b>for each level.<br/>
            </div>
          </Typography>
      }>
        <span style={{height: "70px", display: "inline-block", margin: "10px", padding: '0'}}>
          <Button
            ...
            {props.name} <br /> <hr />
            <p className="pricetext"> Level: {props.level}/19 | Cost: {props.level >= 19 ? " - - - " : addcomma(formatNumber(props.cost))}</p>
          </Button>
        </span>
      </HtmlTooltip>
    </div>
