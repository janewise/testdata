...
const EnergyFill: FunctionComponent<ButtonProps> = (props: ButtonProps) => {
  ...
  if (props.balance >= props.cost - Math.max(200, props.cost * 0.2) && !isVisible) {
    setIsVisible(true);
  } if(props.level >= 4 && !isVisible){
    setIsVisible(false);
  }

  useEffect(() => {
    const balanceCondition = props.cost <= props.balance;
    const levelCondition = props.level < 4;
    ...
    <div className="upgrade_box">
      <HtmlTooltip placement={isMobile ? 'bottom' : 'left'} title={
          <Typography variant="body2" align="left">
            {props.name} <br/>
            Max: {props.level}/4 <br/>
            <div style={{display: props.level === 0 ? "none": "inline-block"}}>
              <hr />
              {"Increase +" + props.level+" energy per second"}.<br/>
            </div>
          </Typography>
      }>
        <span style={{height: "70px", display: "inline-block", margin: "10px", padding: '0'}}>
          <Button
            ...
            {props.name} <br /> <hr />
            <p className="pricetext"> Level: {props.level}/4 | Cost: {props.level >= 4 ? " - - - " : addcomma(formatNumber(props.cost))}</p>
          </Button>
        </span>
      </HtmlTooltip>
    </div>
